...
import { Card, Button, Badge, cn, LoadingSpinner, EmptyState } from "../components/ui";
...
export const DashboardPage = React.memo(() => {
  ...
  const memoizedRecentTxs = useMemo(() => recentTxs, [recentTxs]);
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <EmptyState 
          ...
          description="Please select or create a business in settings to view your dashboard."
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => navigate("/transactions")}><Search className="w-4 h-4 mr-2" /> Search</Button>
          <Button onClick={() => navigate("/transactions")}><Plus className="w-4 h-4 mr-2" /> New Transaction</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="Recent Transactions">
          <div className="space-y-4 mt-4">
            {recentTxs.length === 0 ? (
              <EmptyState 
                ...
                description="Your recent transactions will appear here once you start recording them."
              />
            ) : (
              recentTxs.map((tx) => (
                <div key={tx.id} onClick={() => navigate("/transactions")} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-xl transition-colors cursor-pointer group">
          ...
          {recentTxs.length > 0 && (
            <Button variant="ghost" className="w-full mt-4 text-indigo-600" onClick={() => navigate("/transactions")}>
        ...
        <Card title="Quick Actions">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
            <Button variant="outline" className="h-24 flex-col gap-2" onClick={() => navigate("/transactions")}>
            ...
            <Button variant="outline" className="h-24 flex-col gap-2" onClick={() => navigate("/payroll")}>
